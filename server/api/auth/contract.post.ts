...
export default defineEventHandler(async (event) => {
  ...
  if (contract) {
    ...
    const imported = await importContractFromLegacy(contractNumber, password)

    if (!imported) {
      // Гонка: другой запрос мог импортировать договор — пробуем ещё раз локально
      contract = await prisma.contract.findFirst({
        where: { contract_number: contractNumber, contract_password: password },
        select: {
          id: true, owner_user_id: true, contract_number: true, balance: true,
          status: true, is_blocked: true, pay_day: true, address_full: true, start_date: true
        }
      })
      if (!contract) {
        throw createError({ statusCode: 401, message: 'Неверный номер договора или пароль' })
      }
      // Договор нашёлся после гонки — берём данные
      user = await prisma.user.findUnique({
        where: { id: contract.owner_user_id },
        select: {
          id: true, first_name: true, last_name: true, middle_name: true,
          email: true, phone: true, telegram_id: true, telegram_username: true,
          birth_date: true, avatar: true, vk_id: true, status: true
        }
      })
      if (!user) {
        throw createError({ statusCode: 404, message: 'Владелец договора не найден' })
      ...
      contractServices = imported.contractServices
    }
  }
